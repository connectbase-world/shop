...
function NavigationsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await cb.database.createData(NAVIGATIONS_TABLE_ID, {
        data: {
          name,
          slug,
          is_active: true,
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mb-6 p-4 bg-gray-50 rounded-lg space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium mb-1">슬러그 *</label>
              <input
                type="text"
                value={slug}
      ...
      {navigations.length === 0 ? (
        <div className="text-center py-16 text-gray-400 text-sm">
          ...
          <p>네비게이션이 없습니다</p>
          <p className="text-xs mt-1">Shop 헤더/푸터에 표시할 메뉴를 만들어보세요</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-4 py-3 font-medium w-32">슬러그</th>
            ...
            <tbody>
              {navigations.map((nav) => (
                <tr key={nav.id} className="border-b border-gray-50 hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium">{nav.name}</td>
                  <td className="px-4 py-3 text-gray-500">
                    <code className="text-xs bg-gray-100 px-1.5 py-0.5 rounded">{nav.slug}</code>
                  </td>
